...
func (p *PodmanProvider) Build(ctx context.Context, opts BuildOptions) error {
	args := []string{"build"}
...
func (p *PodmanProvider) RunInteractive(ctx context.Context, spec ContainerSpec) error {
...
func (p *PodmanProvider) StreamLogs(ctx context.Context, containerID string, w io.Writer, opts LogOptions) error {
	args := []string{"logs"}
	if opts.Follow {
		args = append(args, "-f")
	}
